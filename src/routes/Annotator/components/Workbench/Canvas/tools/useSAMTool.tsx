...
const useSAMTool = () => {
  ...
  const setChildrenWithSegmentation = useCallback(
    (segmentation: number[][][], correction: number[]) => {
    ...
    [currentAnnotation?.annotationId, currentCategory?.categoryId],
  );
  ...
  const click = useCallback(
    ...
    [image, dispatch, coords, labels, setChildrenWithSegmentation],
  );

  // view.bounds와 raster.bounds의 교차점을 구함
  const getRegion = useCallback(
    (viewBounds: paper.Rectangle, rasterBounds: paper.Rectangle) => {
  ...
  const getConvertedCoordinate = useCallback(
    (topLeft: paper.Point, bottomRight: paper.Point, raster: paper.Raster) => {
      return [
  ...
  const loadFinetunedModel = useCallback(
    async (finetuneId: number) => {
      dispatch(setSAMModelLoading(true));
      try {
  ...
  const tool = useMemo(() => {
    const tool = new AnnotationTool(Tool.SAM);
